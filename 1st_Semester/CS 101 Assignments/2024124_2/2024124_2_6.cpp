...
bool verifyPin(int accountNum, int enteredPin, int pins[]) {
    int index = accountNum - 1001;
    return index >= 0 && index < MAX_NUM_ACCOUNTS && pins[index] == enteredPin;
}
...
int main() {
    ...
    int accountPins[MAX_NUM_ACCOUNTS] = {0};
    int totalAccounts = 0;
    ...
    while (true) {
        ...
        switch (transType) {
            ...
            case 'B': {
                ...
                if (isAccountValid(accountNum, accountStatus) && verifyPin(accountNum, pin, accountPins)) {
                    cout << "Balance: " << accountBalances[accountNum - 1001] << endl;
                } else {
                    cout << "ERROR: ACCOUNT NOT FOUND OR INVALID PIN\n";
                }
                break;
            }
            ...
            case 'C': {
                ...
                if (isAccountValid(accountNum, accountStatus) && verifyPin(accountNum, pin, accountPins)) {
                    accountStatus[accountNum - 1001] = false;
                    accountBalances[accountNum - 1001] = 0; // Set balance to 0
